...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.use_comet = self.config.comet_model.enable

        if self.use_comet:
            if (n := self.config.comet_model.get("n", 0)) <= 0 or len(m_list := list(self.config.comet_model.get("model_dict", {}).items())) <= 0:
                raise ValueError(f"You wants to use comet model as rm, but use n = {n}, and model_list = {m_list}")
            if n < len(m_list):
                print(f"we just use the first {n} models in {m_list} as rm model, all of them will be used to validate.")
            elif n > len(m_list):
                raise ValueError(f"You wants to use comet model as rm, but use n = {n}, and model_list = {m_list}, then n > {len(m_list)}")
    ...
    def init_workers(self):
        ...
        if self.use_comet:
            ...
            _n = self.config.comet_model.get("n", 1)
            n = len(list(self.config.comet_model.model_dict.items()))
            for i in range(n):
                name = getattr(Role, f"COMET_{i}")
                resource_pool = self.resource_pool_manager.get_resource_pool(name)
                OmegaConf.set_struct(self.config, True)
                with open_dict(self.config.comet_model):
                    (
                        self.config.comet_model.model_name,
                        self.config.comet_model.ckpt_path
                    ) = list(self.config.comet_model.model_dict.items())[i]
                    if i + 1 <= _n:
                        print(f"\033[92muse {self.config.comet_model.model_name} as our reward+valid model\033[0m")
                    else:
                        print(f"\033[91muse {self.config.comet_model.model_name} as our valid model\033[0m")
                    comet_cls = RayClassWithInitArgs(self.role_worker_mapping[name], config=self.config.comet_model)
                    self.resource_pool_to_cls[resource_pool][str(name)] = comet_cls
        ...
        if self.use_comet:
            self.comet_wg_list = []
            n = self.config.comet_model.get("n", 1)
            for i in range(n):
                na = getattr(Role, f"COMET_{i}")
                self.comet_wg_list.append(all_wg[str(na)])
                self.comet_wg_list[-1].init_model()
    ...
    def _start_profiling(self, do_profile: bool) -> None:
        ...
        if do_profile:
            ...
            if self.use_comet:
                for i in range(len(self.comet_wg_list)):
                    self.comet_wg_list[i].start_profile(profile_step=self.global_steps)
    ...
    def _stop_profiling(self, do_profile: bool) -> None:
        ...
        if do_profile:
            ...
            if self.use_comet:
                for i in range(len(self.comet_wg_list)):
                    self.comet_wg_list[i].stop_profile()
